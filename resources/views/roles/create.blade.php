<x-tomato-admin-container label="{{trans('tomato-admin::global.crud.create')}} {{trans('tomato-roles::global.roles.single')}}">
    <x-splade-form class="flex flex-col space-y-4" action="{{route('admin.roles.store')}}" method="post" :default="['permissions' => []]">
